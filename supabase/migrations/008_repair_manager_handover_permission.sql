...
begin
  ...
  if v_actor_role not in ('manager', 'head') then
    raise exception 'Only bank manager/head can complete manager handover.';
  end if;

  if p_new_period_name is null or length(trim(p_new_period_name)) < 2 then
    raise exception 'New manager period name is required.';
  end if;

  if not exists (select 1 from public.profiles where id = p_new_manager_user_id and role = 'manager') then
    raise exception 'Select a valid bank manager for the new period.';
  end if;
  ...
  if v_old.id is not null then
    ...
    set is_active = false,
        ended_at = now(),
        closing_liquid_balance = v_cash.liquid_balance,
        closing_online_balance = v_cash.online_balance
    where id = v_old.id;
  end if;

  insert into public.manager_periods(name, manager_user_id, is_active, opening_liquid_balance, opening_online_balance)
  values (trim(p_new_period_name), p_new_manager_user_id, true, v_cash.liquid_balance, v_cash.online_balance)
  ...
  select id, auth.uid(), 'Manager handover completed', 'A new manager period has started: ' || trim(p_new_period_name), '/manager'
  from public.profiles
  where role in ('manager','head');

  perform public.log_audit('manager.handover', 'manager_periods', v_new_id, jsonb_build_object('report_id', v_report_id, 'completed_by_role', v_actor_role));
